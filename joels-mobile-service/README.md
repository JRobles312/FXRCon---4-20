# Joel's Mobile Service — Website & Phone App

A single-page website + installable phone app (PWA) for Joel, a mobile
...
- **Book** — a working calendar. Clients pick an open day + time and submit a
  request. Joel can **log in ("Mechanic login")** to block whole days,
...
- **Oil Reminders** — clients enter their last oil change + oil type; the app
  ...
  can save the reminder for Joel to follow up.
- **Pay** — Zelle-based. Shows Joel Zelle recipient with a copy button
...
- **Clients (mechanic-only)** — a client book behind the mechanic PIN:
  ...
  - **Booking calendar** shows client requests as *Requested* for Joel to
...
const CONFIG = {
  ...
  email: "joel@example.com",    // where the contact form is delivered
  adminPin: "0420",                 // PIN for "Mechanic login" on the Book page
  formEndpoint: "",                 // see "Contact form" below
  pay: { zelle:"", zelleName:"Joel" },    // Zelle recipient (email or phone)
...
message pre-filled to Joel. Email links can't carry photo attachments, so
...
then emailed straight to Joel — no manual attaching needed.

### Privacy — email & Zelle are shielded
Joel **email is never shown as text** and there's no static `mailto:`
...
address or phone number Joel Zelle is registered to (e.g.
`"joel@example.com"` or `"(408) 555-0123"`), and optionally `zelleName`.
...
Joel later gets a **PartsTech** account (an aggregator with a real
...
single phone/tablet that Joel uses as his business device. Two implications:
...
- Reminder **texts are one-tap, not automatic** — the app surfaces who's due and
  pre-writes the SMS, and Joel taps send. To sync across devices or send
